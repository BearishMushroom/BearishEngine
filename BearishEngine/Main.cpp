...
#include <BE/Graphics/API/ShaderStage.h>
...
int main(int argc, char** argv) {
	{
		...
		API::ShaderStage vertex(&vsh, API::ShaderStage::Type::Vertex);
		API::ShaderStage fragment(&fsh, API::ShaderStage::Type::Fragment);

		// Pipeline: vertex input
		VkPipelineVertexInputStateCreateInfo vertexInputInfo = {};
		vertexInputInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO;
		// For testing vertex attribs are hardcoded.
		...
		viewport.x = 0.0f;
		viewport.y = 0.0f;
		viewport.width = (f32)swapchain.GetWidth();
		viewport.height = (f32)swapchain.GetHeight();
