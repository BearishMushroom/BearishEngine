...
namespace Bearish { namespace Graphics {
	...
	template<typename T>
	class BEARISH_API VertexLayout : Core::IAllocatable<VertexLayout<T>> {
	public:
		static VertexLayout<T> Default;

		VertexLayout() : _size(sizeof(T)), _offset(0) {}
		VertexLayout(Core::Action<VertexLayout&> init) : _size(sizeof(T)), _offset(0) { init(*this); }
		~VertexLayout() {}
