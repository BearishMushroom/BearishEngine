...
namespace Bearish { namespace Graphics {
	class MeshAnimation : public Core::IAllocatable<MeshAnimation> {
	public:
		MeshAnimation(string name, f32 tickRate, f32 duration, std::vector<MeshNodeAnimation*> channels, MeshSkeleton* skeleton) :
			_active(false), _name(name), _skeleton(skeleton), _tickRate(tickRate), _duration(duration), _time(0), _channels(channels) {}
		...
		MeshNodeAnimation* GetNodeAnimation(string name);
